...
| [ADR-017](ADR-017-log-and-catalog-entry-format-enforcement.md) | Structural Format Enforcement for `log.md` and `index.md` Entries | Accepted | Extends ADR-006, ADR-015, ADR-016 (no supersession) | Amended by ADR-027 (proposed — `log.md` ordering only) |
...
| [ADR-027](ADR-027-agent-owned-activity-log-prepend-ordering.md) | Agent-Owned Activity Log — Prepend-Only Ordering and Removal of Harness Authorship | Proposed | Amends ADR-017 (`log.md` ordering and backstop bullet only) | — |
