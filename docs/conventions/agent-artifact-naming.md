...
| `HarnessSurfaceGrantProvenanceTests` | 022-align-wiki-structure Phase 5 (T067): exercises all three ADR-023 provenance records side by side — the Ingest task artifact, the cross-agent terminal NDJSON event, and the Query conversation record; `Grimoire.Hub.QueryConversations` is not in Part 1's reference-detection prefixes, so without this exemption the scan would see only the Ingest reference (same precedent as `SiblingDirectoryLayoutTests`) |
