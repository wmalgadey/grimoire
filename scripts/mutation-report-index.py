...
def band(score: float | None) -> str:
    if score is None:
        return "none"
    return "high" if score >= 80 else "low" if score >= 60 else "poor"
...
def main(argv: list[str]) -> int:
    ...
    worst = sorted(
        (f | {"target": t["name"]} for t in targets for f in t["files"]),
    ...
    rows = "\n".join(
        ...
        <td class="num {band(t['score'])}">{t['score']:.1f}&thinsp;%</td>
        ...
        <td class="num">{t['counts']['CompileError']}</td>
    ...
    worst_rows = "\n".join(
        ...
        <td class="num {band(f['score'])}">{f['score']:.1f}&thinsp;%</td>
      </tr>"""
        for f in worst
    )
...
<main>
  ...
  <table>
    <thead>
      ...
      <th class="num">Compile err.</th><th class="num">Scored</th></tr>
    </thead>
    <tbody>
{rows}
    </tbody>
...
{worst_rows}
  ...
  killed — the mutant broke the code badly enough to hang a test. Compile errors are
  outside the score, but a large count means Stryker's Safe Mode dropped whole methods
  from the run.</footer>
</main>
</body>
</html>
