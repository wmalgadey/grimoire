...
- [ ] TXXX Agent-behavior evaluation completeness audit (MANDATORY for features with agentic behavior — Constitution Principles II, III & V): confirm every **high-stakes** agent-judgment success criterion from spec.md has a passing evaluation test, at its defined threshold via sampled runs with real or recorded LLM output — wherever that test lives — confirm every **lower-stakes** agent-judgment success criterion is covered by a hermetic test of the surrounding harness plumbing and recorded as subject to the user-reported correction loop rather than a formal eval suite, and file any gap found as a new task before declaring the DoD met
