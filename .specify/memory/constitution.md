...
Templates assessed (in addition to the original entry above):
  ...
  - `docs/adr/index.md` — assessed, no change required by this amendment: its existing legend text already documents `Extends` informally and is not contradicted by the new frontmatter contract; retrofitting its legend and the 34 existing ADR rows to the new frontmatter fields is deferred alongside the retroactive-splitting TODO above.
