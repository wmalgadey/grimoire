...
/// record.
/// </summary>
public class QueryConversationRecordDurabilityTests
{
    [Fact]
    public async Task Restart_RecordContainsFinishedTurnsByteComplete_AndKilledTurnWithPartialAnswer()
    {
        ...
        byte[] bytesBeforeRestart;
        {
            ...
            var killedTurnId = await QueryConversationRecordLifecycleTests.SubmitAsync(client, "c-durable", "Third, killed midway?");
            ...
            await QueryConversationRecordLifecycleTests.WaitForAnswerAsync(client, killedTurnId);
            await QueryConversationRecordLifecycleTests.WaitForStateAsync(client, killedTurnId, "failed");
    ...
    public async Task FollowUpAfterRestart_HydratesContextFromTheRecordFile_TupleEqualAndSourceRecord()
    {
...

        {
            ...
            await QueryConversationRecordLifecycleTests.WaitForAnswerAsync(client, turn2);
            (await client.PostAsync($"/api/query-turns/{turn2}/interrupt", content: null)).EnsureSuccessStatusCode();
            await QueryConversationRecordLifecycleTests.WaitForStateAsync(client, turn2, "interrupted");
