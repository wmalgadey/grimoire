...
public abstract record QueryConversationContextResult
{
    /// <summary><c>Source</c> is <c>memory</c> (cache hit), <c>record</c> (hydrated from file), or <c>empty</c> (new conversation).</summary>
    public sealed record Loaded(IReadOnlyList<QueryPriorTurn> Turns, string Source) : QueryConversationContextResult;

    /// <summary>The record file exists but is structurally unreadable — fail closed, never partial context (FR-006).</summary>
    public sealed record Unreadable(string Reason) : QueryConversationContextResult;
}
...
/// </summary>
public sealed class QueryConversationRecordStore
{
    private readonly ResolvedGrimoirePaths _paths;
    private readonly TimeProvider _timeProvider;
    private readonly ILogger<QueryConversationRecordStore> _logger;
    ...
    public QueryConversationRecordStore(
        ResolvedGrimoirePaths paths,
        TimeProvider? timeProvider = null,
        ILogger<QueryConversationRecordStore>? logger = null)
    {
        _paths = paths;
        _timeProvider = timeProvider ?? TimeProvider.System;
        _logger = logger ?? NullLogger<QueryConversationRecordStore>.Instance;
    }
    ...
    /// </summary>
    public async Task AppendTurnAsync(string conversationId, QueryRecordedTurn turn, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!File.Exists(path))
            {
                ...
                var content = QueryConversationRecordFormat.BuildRecordHeader(conversationId, _timeProvider.GetUtcNow())
                              + QueryConversationRecordFormat.BuildTurnBlock(turn);
                await File.WriteAllTextAsync(path, content, QueryConversationRecordFormat.Encoding, cancellationToken);
                _contextCache[conversationId] = [];
                QueryConversationRecordLogEvents.LogRecordCreated(_logger, conversationId, path);
            }
            else
            {
                ...
                await File.AppendAllTextAsync(
                    path, QueryConversationRecordFormat.BuildTurnBlock(turn), QueryConversationRecordFormat.Encoding, cancellationToken);
            }
            ...
            QueryConversationRecordLogEvents.LogTurnRecorded(_logger, conversationId, turn.TurnId, turn.Position, turn.State);
    ...
    /// fail-closed <see cref="QueryConversationContextResult.Unreadable"/> — never partial
    /// context (research.md R5, FR-006).
    /// </summary>
    public async Task<QueryConversationContextResult> LoadContextAsync(string conversationId, CancellationToken cancellationToken = default)
    {
        if (_contextCache.TryGetValue(conversationId, out var cached))
        {
            QueryConversationRecordLogEvents.LogContextLoaded(_logger, conversationId, cached.Count, "memory");
            HubMetrics.RecordQueryConversationContextLoad("memory");
            return new QueryConversationContextResult.Loaded(cached, "memory");
        }
        ...
        try
        {
            ...
            if (!File.Exists(path))
            {
                QueryConversationRecordLogEvents.LogContextLoaded(_logger, conversationId, 0, "empty");
                HubMetrics.RecordQueryConversationContextLoad("empty");
                return new QueryConversationContextResult.Loaded([], "empty");
            }

            var content = await File.ReadAllTextAsync(path, QueryConversationRecordFormat.Encoding, cancellationToken);
            switch (QueryConversationRecordFormat.Parse(content))
            {
                case QueryConversationRecordParseResult.Parsed parsed:
                    {
                        if (parsed.DroppedTrailingFragment)
                        {
                            QueryConversationRecordLogEvents.LogTrailingFragmentDropped(_logger, conversationId);
                        }

                        IReadOnlyList<QueryPriorTurn> turns = [.. parsed.Turns.Select(t => t.ToPriorTurn())];
                        _contextCache[conversationId] = turns;
                        QueryConversationRecordLogEvents.LogContextLoaded(_logger, conversationId, turns.Count, "record");
                        HubMetrics.RecordQueryConversationContextLoad("record");
                        return new QueryConversationContextResult.Loaded(turns, "record");
                    }

                case QueryConversationRecordParseResult.Unreadable unreadable:
                    QueryConversationRecordLogEvents.LogRecordLoadFailed(_logger, conversationId, unreadable.Reason);
                    HubMetrics.RecordQueryConversationRecordLoadFailure();
                    return new QueryConversationContextResult.Unreadable(unreadable.Reason);
    ...
    {
        var content = await File.ReadAllTextAsync(path, QueryConversationRecordFormat.Encoding, cancellationToken);
        if (QueryConversationRecordFormat.Parse(content) is not QueryConversationRecordParseResult.Parsed parsed)
        {
            return false;
        }
