...
**Independent Test**: Trigger a lint run that surfaces at least one issue worth fixing, and confirm that once the agent has assessed the findings, a separate task card appears on the board for each proposed remediation action, distinct from the lint run's own card.

**Acceptance Scenarios**:

1. **Given** a lint run completes and identifies one or more issues worth fixing, **When** the agent finishes assessing them, **Then** one task card appears on the board per proposed remediation action, each describing what the agent proposes to do.
...
- What happens if two proposed remediation actions from the same lint run touch overlapping wiki content? Each must still be reviewable and decidable independently; the system must not silently merge or drop either one.
...
- **FR-004**: The system MUST block starting a new lint run while a lint run is already active, or while any remediation action task from a prior run has not reached a terminal outcome (completed, failed, or dismissed) — and MUST clearly tell the user why, rather than silently ignoring the request.
...
- **FR-007**: When a lint run identifies issues worth acting on, the agent itself MUST assess the findings and propose one remediation action task per identified action, each surfaced as its own board entry; this judgment MUST be exercised by the agent, not derived by fixed backend rules.
...
- **Lint Run**: An execution of the wiki health check. Has a status that moves through in-progress to a terminal outcome (completed or failed), a start time, and, once finished, either a findings assessment (which may produce remediation action tasks) or a failure reason.
- **Remediation Action Task**: A single, agent-proposed fix arising from a lint run's findings. Has a description of what it proposes to do, a state that moves from proposed, through optional human-added context and agent discussion, to authorized, and then to a terminal outcome (completed, failed, or dismissed without ever being authorized).
