...
- [X] T039 [US4] Agent-behavior evaluation (FR-018) in `backend/tests/Grimoire.AgentEvals/RemediationReVerificationEvalTests.cs`: fixtures pairing a recorded proposal with changed vs. unchanged wiki states; agent chooses apply vs. not-applicable correctly in ≥ 90% of sampled runs (ADR-012 replay). **Closed 2026-08-01**: live-captured with `claude-haiku-4-5` via `dotnet run --project backend/src/Grimoire.EvalRunner -- capture --scenario remediation-reverify-still-applicable --scenario remediation-reverify-no-longer-applicable` — 100% success rate on both scenarios (threshold 90%).
  - **Deviation note (left unchecked)**: unlike T028 (which reused the existing lint-run replay/capture pipeline for a new scorer), FR-018 exercises the genuinely new remediation-execution invocation mode T035 added, so this task also built that mode's own capture/replay pipeline pair (`Grimoire.EvalRunner.Capture.RemediationReVerificationCapturePipeline`, `Grimoire.EvalRunner.Replay.RemediationReVerificationReplayPipeline`), staleness check (`RemediationReVerificationStalenessCheck`), process invoker (`LintAgentProcessInvoker.RunRemediationExecutionAsync`), scenario pair (`RemediationReVerificationScenarioDefinitions.StillApplicable`/`.NoLongerApplicable`), two fixture wikis (`backend/tests/Grimoire.AgentEvals/Fixtures/remediation-reverify-{still-applicable,no-longer-applicable}/`), a fully deterministic scorer (`RemediationReVerificationScorer` — objective per-fixture expected outcome, no golden-set stand-in needed, unlike T028's SC-006 scorer; hermetic coverage in `RemediationReVerificationScorerTests.cs`), and `capture`/`replay`/`status` CLI wiring in `Grimoire.EvalRunner/Program.cs`. The harness is fully implemented and wired end-to-end (`dotnet run --project backend/src/Grimoire.EvalRunner -- capture --scenario remediation-reverify-still-applicable --scenario remediation-reverify-no-longer-applicable` will genuinely capture once credentials exist). What is missing is the recorded fixture itself: this environment has no LLM provider credentials, so no live capture could be run (Constitution II forbids fabricating recorded evidence) — both tests fail honestly with a `Missing`-trust message naming the exact capture command above (needs `GRIMOIRE_EVAL_PROVIDER_API_KEY`/`ANTHROPIC_AUTH_TOKEN` per the existing eval-capture workflow).
