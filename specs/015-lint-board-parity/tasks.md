...
- [X] T020 [P] [US3] Hermetic integration test in `backend/tests/Grimoire.IntegrationTests/LintRemediationProposalMaterializationTests.cs` (renamed from `RemediationProposalMaterializationTests.cs` — ADR-013/N1 naming rule: the class references `Grimoire.Hub.LintDispatch` directly and must therefore carry the `Lint` token): fake lint agent completes with N `proposedActions` → N `remediation_tasks` rows in `Proposed` + N task records exist **before** the lint run's terminal broadcast is published (FR-007 ordering); verbatim title/description (no harness rewriting, Principle V); empty/absent list → zero rows, run still completes; `remediationTaskLifecycleChanged` broadcast per new card on `/hubs/remediation-lifecycle`.
- [X] T021 [P] [US3] Hermetic integration test in `backend/tests/Grimoire.IntegrationTests/BoardCompositeResponseTests.cs` (extend): `remediation_task` typed entries appear with proposal title, originating-run subtitle, state, detailLink; independently listable per contracts/remediation-task-api.md (list + get endpoints).

### Implementation for User Story 3

- [X] T022 [US3] Materialize proposals in `backend/src/Grimoire.Hub/LintDispatch/LintRunCoordinator.cs` `FinishRunAsync`: parse `proposedActions` off the terminal event, generate task ids (`{date}-remediation-{guid}` per data-model.md), insert `Proposed` rows via `OperationalStateRepository`, create task records via `RemediationTaskRecordStore` — all before `TryTransitionTo(Completed)`/lifecycle publish. Emit span `hub.lint.propose_remediation_tasks` (parent `hub.lint.run_supervision`, attrs `run_id`, `proposed_count`), log event `hub.lint.remediation_task_proposed` (INFO, `run_id`, `task_id`) per proposal, metric `wiki.lint.remediation_tasks_proposed_total`.
- [X] T023 [US3] Implement `RemediationLifecycleHub` (route `/hubs/remediation-lifecycle`) + `RemediationLifecyclePublisher` + `RemediationLifecycleLogEvents` in `backend/src/Grimoire.Hub/RemediationTasks/`, events per contracts/remediation-lifecycle-events.md; metric `hub.remediation_lifecycle_updates_total{stage}` in `backend/src/Grimoire.Hub/HubMetrics.cs`; wire into `Program.cs`.
- [X] T024 [US3] Implement list/detail endpoints of contracts/remediation-task-api.md (`GET /api/remediation-tasks`, `GET /api/remediation-tasks/{taskId}` incl. record-derived history) in `backend/src/Grimoire.Hub/RemediationTasks/RemediationTaskEndpoints.cs`; fold `remediation_task` entries into the composite board response (extend T012's endpoint).
- [X] T025 [US3] Extend `agents/lint/system-prompt.md`: findings assessment now also proposes remediation actions — instruct the agent to judge which findings are actionable (informational findings produce none) and emit one `proposedActions` entry per action on its completion event; extend `backend/src/Grimoire.LintAgent` to carry the agent's proposals onto the terminal NDJSON event (loop mechanics only — content stays agent-authored, Principle V).
- [X] T026 [P] [US3] Frontend: `remediationApi.ts` + `remediationLifecycleClient.ts` in `frontend/src/lib/services/`, `RemediationTaskCard.svelte` in `frontend/src/lib/components/` (Proposed card: proposal text, review actions placeholder, visually distinct per FR-006), board integration in `frontend/src/routes/+page.svelte`; independent review of each card (US3 scenario 3).
- [X] T027 [P] [US3] Deterministic observability tests in `backend/tests/Grimoire.IntegrationTests/LintRemediationObservabilityTests.cs` (renamed from `RemediationObservabilityTests.cs`, same ADR-013/N1 reason as T020): event name/level/fields for `hub.lint.remediation_task_proposed`; span `hub.lint.propose_remediation_tasks` with parent linkage + `run_id`/`proposed_count`; metric `wiki.lint.remediation_tasks_proposed_total` (in-memory exporter, ADR-005 pattern).
- [X] T028 [US3] Agent-behavior evaluation (SC-006) in `backend/tests/Grimoire.AgentEvals/LintRemediationProposalRelevanceEvalTests.cs` (renamed per the ADR-013/N1 naming rule, same reason as T020/T027): sampled lint runs over seeded wiki fixtures via the ADR-012 recorded-replay harness; ≥ 90% of proposed tasks judged relevant/actionable against the human-adjudicated golden set; record fixtures per the existing eval-capture workflow. **Closed 2026-08-01**: live-captured with `claude-haiku-4-5` via `dotnet run --project backend/src/Grimoire.EvalRunner -- capture --scenario lint-remediation-proposals` — 100% success rate (threshold 90%).
  - **Deviation note (left unchecked)**: the harness — scenario `LintScenarioDefinitions.RemediationProposalsRelevant` ("lint-remediation-proposals"), scorer `lint-remediation-proposals-relevant`, golden set `Grimoire.EvalRunner.Scoring.RemediationGoldenSet`, `proposedActions` carriage through `LintAgentProcessInvoker`/`LintSampleRunData`, hermetic scorer-logic tests in `LintDeterministicScorersTests` — is fully implemented. What is missing is the recorded fixture itself: this environment has no LLM provider credentials, so no live capture could be run (Constitution II forbids fabricating recorded evidence). The test fails honestly with a `Missing`-trust message naming the exact command to run: `dotnet run --project backend/src/Grimoire.EvalRunner -- capture --scenario lint-remediation-proposals`. **Also note**: T025's `agents/lint/system-prompt.md` extension changed the `system_prompt` fingerprint that `LintReplayEvalTests` (013-lint-agent, SC-005/SC-006/SC-007/SC-008) checks recordings against — those four scenarios' existing recordings are now `Stale`, not just `lint-inbound-links-refreshed`'s pre-existing threshold failure. Recapture all five scenarios in one pass once credentials are available (the `--scenario` flag repeats): `dotnet run --project backend/src/Grimoire.EvalRunner -- capture --scenario lint-defects-found --scenario lint-genuine-findings --scenario lint-metadata-proposals --scenario lint-inbound-links-refreshed --scenario lint-remediation-proposals`.
...
- [X] T045 Observability completeness audit (MANDATORY — Constitution III/IV): cross-reference every row of plan.md ## Observability (8 metrics, 7 log events, 6 spans) against its implementing task (T022/T023/T032/T033/T041) and passing test (T027/T038/T044) — file any gap found as a new task before declaring the DoD met. **Result: all 21 rows verified present (source + assertion, literal string grep, not task-description trust). No gaps found; no new task filed.**
- [X] T046 Logging + trace contract CI enforcement (MANDATORY — Constitution IV): verify `.github/workflows/ci.yml`'s standard PR pipeline runs `Grimoire.IntegrationTests` (incl. `RemediationObservabilityTests`, `LintLifecycleHubTests`) and `Grimoire.ArchTests` (T001/T002); adjust filters if any new test class is excluded. **Result: ci.yml runs `dotnet test backend/tests/Grimoire.IntegrationTests`/`Grimoire.ArchTests`/`Grimoire.Domain.UnitTests`/`Grimoire.AgentEvals` with no `--filter`, so every new class (all 24 added test files, all in pre-existing projects already wired into `Grimoire.slnx`) runs unfiltered; no `[Fact(Skip=...)]` anywhere in the new files. No workflow change needed. See T047 for the separate, pre-existing eval-suite pass/fail gap this does NOT paper over.**
- [X] T047 Agent-behavior evaluation completeness audit (MANDATORY — Constitution II & V): confirm SC-006 (T028) and FR-018 (T039) evals pass at ≥ 90% via the ADR-012 harness (`.github/workflows/eval.yml` or the documented replay path); file any gap as a new task. **Result: scaffolding confirmed complete and correctly wired (harness, scorers, CLI, hermetic scorer-unit-tests all real); recordings do not exist, so the tests fail honestly (not skip) with the exact capture command named. This is a live, un-closed DoD gap — see the T047 gap statement in the phase report; not resolved by this audit, T028/T039 remain unchecked.**
- [X] T048 Ingest regression check (SC-008/FR-015): full existing `Grimoire.IntegrationTests` ingest suite + existing frontend board tests pass unmodified; diff-review that no ingest source file changed beyond the additive board-response composition (T012/T024). **Result: 147/147 ingest-filtered integration tests pass; `git diff main..015-lint-board-parity` under the ingest-adjacent paths shows exactly one changed path, `backend/src/Grimoire.Hub/IngestSubmission/BoardEndpoints.cs` (new, additive); `IngestSubmissionEndpoints.cs` and `KanbanBoardProjection*.cs` are byte-for-byte untouched (empty diff). PASS.**
- [X] T049 [P] Run quickstart.md validation scenarios end-to-end and record outcomes in the PR description. **Result: Scenarios 1-5 mapped to their automated hermetic/component tests, all passing; live-UI clicking and the Observability-check (Aspire Dashboard) were not run interactively in this environment — reported explicitly as automated-test-mapping only, not a live UI walkthrough. See phase report for the full scenario table.**
- [X] T050 [P] Documentation: update `docs/adr/ADR-018-...md` cross-references if implementation deviated; note the new `remediation-tasks/` runtime directory wherever runtime layout is documented (e.g. README/ops notes if present). **Result: added an "Implementation note" to ADR-018 documenting the T042 second allow-listed caller (`RemediationMessageTurnCoordinator`) alongside `RemediationRunCoordinator`, and why it doesn't weaken SC-005; added a `remediation-tasks/` bullet to `docs/operations/runtime-configuration.md`'s base-level layout section, mirroring the existing `tasks/`/`conversations/` bullets. README.md does not document `tasks/`/`conversations/` either, so no README change was needed for parity.**
...
**Source**: GitHub PR #41 review (Copilot automated review, 2026-08-01) — findings
verified against FR-003/SC-001/SC-002 (live board updates), ADR-018 (SC-005 structural
proof), and T028/T039 (eval scaffolding docs), then appended per `/speckit-converge`.

- [X] T051 Fix `createLintRunStream().start()` in `frontend/src/lib/services/lintLifecycleClient.ts` so the SignalR hub connection starts independently of the `/api/board` bootstrap `refresh()` call, per FR-003/SC-001/SC-002 (contradicts): currently `start()` awaits `refresh()` before `client.start()`, and the board page swallows `start()`'s rejection (`frontend/src/routes/+page.svelte:141`) — a transient board-fetch failure silently and permanently prevents live lint updates until a full page reload, contradicting FR-003's "MUST update... without requiring the user to manually reload the page." Start the hub connection regardless of refresh outcome (e.g. `Promise.allSettled`/independent try-catch per step, refresh failure logged/non-fatal, hub connection attempted either way). Add a regression test exercising a failing bootstrap fetch and asserting the hub connection still starts and live events still apply.
- [X] T052 Apply the same fix as T051 to `createRemediationTaskStream().start()` (or equivalent) in `frontend/src/lib/services/remediationLifecycleClient.ts` per FR-003/SC-002 (contradicts): identical `await refresh(); await client.start();` ordering at the same file's `start()` method (not flagged by the PR's partial automated review, but the same root cause and severity as T051). Add the equivalent regression test.
- [X] T053 Rename `RemediationTasks_OnlyTheRunCoordinator_MayReferenceTheProcessLauncherPort` in `backend/tests/Grimoire.ArchTests/RemediationExecutionDispatchRuleTests.cs` (and any prose referring to "only the RunCoordinator") to accurately name the two-member allow-list (`RemediationRunCoordinator` and `RemediationMessageTurnCoordinator`) per ADR-018 (contradicts) — this test is the literal structural proof of SC-005/FR-008; a misleading name risks a future maintainer misunderstanding or incorrectly narrowing the allow-list. No behavior change, naming/doc-accuracy only.
- [X] T054 Correct the doc comment on the `RemediationProposalsRelevant`-scoring method in `backend/src/Grimoire.EvalRunner/Scoring/LintDeterministicScorers.cs` (~line 182) per T028/SC-006 (contradicts): it currently claims a run with no proposals is "vacuously relevant," but the implementation and its unit tests intentionally fail on an empty `ProposedActions` list for this scenario (correct, since the `lint-remediation-proposals` fixture's seeded defects guarantee the agent should propose something). Update the comment to describe the actual fail-on-empty semantics; no code change.
...
- [X] T056 Restore the `lint-inbound-links-refreshed` eval scenario to its ≥ 95% threshold per Constitution DoD "CI/CD pipeline passes" / T046 (partial). **Root cause**: not a T025 regression — recorded evidence showed the agent miscounting inbound links (e.g. dropping `index.md` from the tally, or attributing an extracted `[[wikilink]]` occurrence to the wrong page / not at all), a pre-existing weakness the T025 fingerprint change merely forced into the open by staling the recording. **Fix**: instruction-only, per Principle V — `data/agents/lint/system-prompt.md` Step 4 now specifies a two-pass counting discipline (pass 1: extract every literal `[[...]]` occurrence per file, ignoring what the surrounding prose claims about link direction; pass 2: attribute each extracted occurrence to exactly one page) plus a conservation sanity-check (summed assigned counts must equal total extracted occurrences) before writing. Iterated three times against live capture until stable: v1 (explicit enumeration) regressed to 50% by inviting semantic reasoning over prose; v2 (mechanical-extraction framing) reached 90% but still dropped occurrences during attribution; v3 (extraction + conservation check) reached **100% (10/10)**. Recaptured `lint-inbound-links-refreshed` plus the four sibling scenarios T028 flagged as `Stale` from the same fingerprint change (`lint-defects-found` 100%, `lint-genuine-findings` 90%, `lint-metadata-proposals` 100%, `lint-remediation-proposals` 100%) and T039's two remediation-reverify scenarios (`remediation-reverify-still-applicable` 100%, `remediation-reverify-no-longer-applicable` 100%) — all committed under `data/evals/recordings/`, all ≥ their respective thresholds. **Verified**: full `Grimoire.AgentEvals` suite 71/71 (was 70/71); `ArchTests` 49/49 and `Grimoire.Domain.UnitTests` 93/93 re-run clean (unaffected by this change, confirmed for completeness); `Grimoire.IntegrationTests` (583/583) and frontend suites (123/123 + svelte-check/eslint/prettier clean) unaffected by this eval-fixture-only change, not re-run.

**Checkpoint**: Full PR pipeline (backend suites incl. AgentEvals 71/71, frontend gates) is green; the 015 DoD is met.
