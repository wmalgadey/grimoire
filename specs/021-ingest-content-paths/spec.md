...
**Feature Branch**: `021-ingest-content-paths`
...
- What happens to the doc comment on `ResolvedGrimoirePaths` that describes it as replacing "the former `ContentRootPaths`"? That comment describes history (what existed before ADR-022) and remains accurate regardless of the current type's name; it is not required to change.
...
- **FR-005**: Every production call site that previously read `SystemPromptPath`, `DefaultUserPromptPath`, or `PolicyPath` from the projection type (including `SubmissionService`, `IngestSubmissionPipeline`, `IngestRunCoordinator`, `IngestSubmissionEndpoints`, `BoardEndpoints`, and the ingest CLI commands `SubmitSourceCommand`, `IngestRetriggerCommand`, `IngestResumeCommand`) MUST be updated to read the same value from `ResolvedGrimoirePaths.Ingest` instead.
- **FR-006**: Every production reference to the old type name — including dependency-injection registration and any constructor, field, or parameter typed as `ContentRootPaths` — MUST be updated to reference the renamed type.
...
- **FR-008**: The solution MUST build, and the full backend test suite MUST pass, with zero remaining references to the identifier `ContentRootPaths` or to the removed field names on the renamed type, anywhere in production or test C# source.
...
- **SC-001**: 100% of production and test C# source files contain zero occurrences of the identifier `ContentRootPaths`.
