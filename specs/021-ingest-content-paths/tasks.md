...
Per plan.md `## Architectural Constraints & ADRs` and `## Constitution Check`: this feature introduces **no new** structural rule — it makes an existing type conform to the naming rule ADR-013 already established, and touches surfaces guarded by ADR-022 and ADR-010. All three ADRs' rules (`AgentArtifactNamingRuleTests`, `NoCodeLevelPathDefaultsRuleTests` / `DirectorySwitchSurfaceRuleTests`, `HexagonalPortsAdapterRuleTests`) already received their Red/Green probes in the features that introduced them. Re-running a fresh Red/Green probe on an already-proven rule would not test anything new; instead this phase confirms the existing guards are live and green immediately before the rename begins, so a subsequent failure is attributable to this feature's change, not a pre-existing gap.

- [X] T001 Run, from `backend/`, the structural guard for every ADR plan.md names as an active constraint: `dotnet test --filter "FullyQualifiedName~AgentArtifactNamingRuleTests|FullyQualifiedName~NoCodeLevelPathDefaultsRuleTests|FullyQualifiedName~DirectorySwitchSurfaceRuleTests|FullyQualifiedName~HexagonalPortsAdapterRuleTests"` — ADR-013's N1 rule (`HubNamespaces_MustFollowTheOwnershipMap`, `ExemptionFixture_MustMirror_TheConventionDocument`), ADR-022's path-surface rules, and ADR-010's port/adapter containment rule. Confirm all pass on the pre-change codebase and record the pass as the baseline this feature must not regress. No file changes in this task.

**Checkpoint**: Existing N1/ADR-022/ADR-010 guards confirmed live. Feature code may now begin.
...
- [X] T017 [US1] Verification checkpoint (depends on T002-T016): run `grep -rn "ContentRootPaths" backend --include="*.cs" | grep -v '^\s*[^:]*:[0-9]*:\s*///'` from repo root and confirm zero output (spec SC-001 — the `///` filter drops the two doc-comment mentions spec.md Edge Cases exempt); run `dotnet build` from `backend/` and confirm it succeeds; run `dotnet test --filter "FullyQualifiedName~AgentArtifactNamingRuleTests"` from `backend/` and confirm it still passes with no exemption-list change (spec SC-005).
...
- [X] T022 [US2] `backend/src/Grimoire.Hub/IngestSubmission/IngestSubmissionEndpoints.cs` — in `GetDefaultsAsync` (~line 209), replace the `IngestContentPaths contentPaths` parameter with `[FromServices] ResolvedGrimoirePaths resolvedPaths` (the `[FromServices]` attribute from `Microsoft.AspNetCore.Mvc` is mandatory — spec FR-010; without it the `GET` handler fails at route-matcher-build time for every host that maps this endpoint group, see T030) and replace the four `contentPaths.DefaultUserPromptPath` reads (~lines 211, 215, 219, 224) with `resolvedPaths.Ingest.DefaultUserPromptPath`. The other three handlers in this file that take `IngestContentPaths` (~lines 243, 269, 373) are unaffected — they only use the five retained fields.
...
- [X] T030 CI-discovered fix (root cause now folded into T022's description and spec FR-010, so a future handler does not repeat it): `backend/src/Grimoire.Hub/IngestSubmission/IngestSubmissionEndpoints.cs` — `GetDefaultsAsync`'s new `ResolvedGrimoirePaths resolvedPaths` parameter (T022) was implicit-bound, which ASP.NET Core's minimal-API `RequestDelegateFactory` eagerly infers as `[FromBody]` when a type's DI-service status can't be confirmed at route-matcher-build time — disallowed on a `GET` handler ("Body was inferred but the method does not allow inferred body parameters"), and thrown eagerly for every test host that maps the ingest-submission endpoint group at all, not only ones that call `/defaults` (`IngestTaskRecordApiTests`, `IngestTaskRecordTraceTests`, `HubRequestTracingTests`, `HubCliParityTests`, `BoardCompositeResponseTests` — 12 failing tests in CI run https://github.com/wmalgadey/grimoire/actions/runs/31185603610/job/92889224829, none of which call `/defaults`). Fixed by adding an explicit `[FromServices]` attribute (`Microsoft.AspNetCore.Mvc`), which defers the DI requirement to actual request time instead of matcher-build time — matching that only `IngestSubmissionPromptApiTests.cs` (T027) actually invokes `/defaults` and needs `ResolvedGrimoirePaths` registered in its host.
...
- **Phase 3 (US1)**: Depends on Phase 0 only. T002-T016 are mutually parallel (15 distinct files); T017 depends on all of them.
