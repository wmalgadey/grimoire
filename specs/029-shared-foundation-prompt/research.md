...
> **Partly superseded by the 2026-09-05 clarification session** (spec.md `## Clarifications`): the
> identity wizard moved out of `deploy/server/grimoire-server` and into the system itself, and the
> drafting agent session stays on the deploy host rather than being started by the system. **R1, R3,
> R4 and R5 are unaffected** — they concern Part 1, which did not change. **R2, R6 and R7 were written
> against the deployment-script wizard and no longer describe the agreed design**; they are left here
> as the record of what was considered and will be regenerated by `/speckit-plan` against the corrected
> spec, not hand-patched (CLAUDE.md: a layer above a changed spec is regenerated, never patched to
> match). R2's *deployment* mechanics — one read-only bind mount driven by one `.env` variable — remain
> the candidate for how an instance document reaches a container; what changes is who writes it.
