...
- Two log entries may share an identical `[DATE] TYPE | SUMMARY` heading for two distinct actions — the append-only invariant preserves both, and they are distinguished by position and by their descriptive paragraphs. SC-004 guarantees every entry is *found* by the heading pattern, not that headings are unique.
- A topical category legitimately named `pages/` is permitted in wiki content — the retirement of the old wrapper folder (FR-001/FR-006) is enforced only against production code (the Phase 0 structural boundary rule), never against agent-chosen content category names.
