...
| SC-005 (≥90% of sampled agent-written log paragraphs specifically/accurately describe the change) | Agent-judgment threshold | Evaluation (recorded replay), threshold 90% | Recorded/replayed `IModelClient` (ADR-012) | Dedicated new scenario `log-paragraph-specificity` on the `empty-topic` fixture (T061 — not a reuse of an existing scenario; the fresh fixture keeps the run's own log entry unambiguously the last one written), recorded live and replaying at 100% | `LogParagraphSpecificityScorer` in `Grimoire.EvalRunner/Scoring`, checking paragraph specificity against the sample's actual touched pages, wired into `CapturePipeline` via `ScorerId`-dispatched judge invocation |
...
| SC-007 (≥90% of sampled catalog descriptions specifically/accurately describe the article) | Agent-judgment threshold | Evaluation (recorded replay), threshold 90% | Recorded/replayed `IModelClient` | Dedicated new scenario `catalog-description-specificity` on the `empty-topic` fixture (T061, same rationale as SC-005 — not a reuse), recorded live and replaying at 100% | `CatalogDescriptionSpecificityScorer`, same wiring pattern as `LogParagraphSpecificityScorer` |
