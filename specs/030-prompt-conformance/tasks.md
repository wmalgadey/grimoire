...
- [X] T025 Run the operator-loop validation in [quickstart.md](./quickstart.md) steps 5 and 6 against a scratch wiki and record the observations on the surfaces `plan.md ## Observability > Operator loop surfaces` names — created-page frontmatter, the `sources/` folder, citation resolution, the findings board at `frontend/src/routes/+page.svelte`, and the degraded run's report (SC-003, SC-004, SC-005, FR-010) — **Done 2026-09-11**: unblocked by the same `.env` credential T023 used. Ran `submit-source` then `lint-run` against a scratch wiki (a short multi-page technical source on vector clocks) via the real `Grimoire.Hub` CLI, then a second `lint-run` with an **instance foundation document** at `<data-dir>/foundation-prompt.md` — a trimmed copy of the shipped one with the Lifecycle field group removed — for step 6.
  - **Step 5, frontmatter (SC-003, FR-002a)**: exactly as expected on every one of the 9 pages the ingest run created — `inbound_links` present with a real count, `last_reviewed` absent. Confirmed by reading the wiki files directly (the findings-board surface itself was not exercised through the SvelteKit UI — no frontend/Hub-API stack was started for this pass — the underlying findings-report record was read directly instead, which carries the same content the board renders).
  - **Step 5, `sources/` folder and citations (SC-005, FR-008)**: the run left exactly one `sources/<slug>.md` page, and every page's `[^1]: [[sources/<slug>]]` citation footnote resolves to it — FR-008 holds precisely as specified.
  - **Step 5, review candidates (SC-004, FR-002b/d/e)**: inconsistent across the two lint runs, recorded as-is rather than smoothed over. The first (non-degraded) run produced one substantive finding tied to a specific page (`lamport-timestamps.md`) but wrote **no** `last_reviewed` anywhere — FR-002b's positive case did not fire. The second (degraded) run wrote `last_reviewed` on 10 of 12 pages, including 3 pages (`hybrid-logical-clocks`, `dotted-version-vectors`, `interval-tree-clocks`) that were never named in any finding, violating FR-002e's exclusion. This is recorded rather than corrected: per Principle II, a single-sample discrepancy in a lower-stakes agent-judgment criterion is the correction loop's normal input, not a blocking defect — it would take more than two samples to tell a real instruction-wording gap from ordinary model variance, and no eval threshold applies here (spec.md SC-004).
  - **Step 5, dangling citations (SC-005 general link health, not FR-008 specifically)**: the pages ingest wrote use natural-language wikilink targets (e.g. `[[Amazon Dynamo]]`, `[[CAP theorem]]`) that mostly do not match the actual page slugs (`dynamo`, `cap-theorem`) the same run created, contradicting the foundation document's own `[[slug]]`/filename-resolution convention — yet Lint's Structure check reported zero orphans, suggesting it resolves the same way (by title) rather than by the documented rule. Not caused by this feature (wikilink resolution is untouched by FR-001–FR-012) and out of this feature's scope per the same "found while verifying, filed rather than fixed" precedent as T029 — filed as **#251**.
  - **Step 6, degraded run (FR-010/FR-010a)**: the harness-completion half holds cleanly — the run completed (exit 0), the instance foundation document was resolved and used (`source=instance` in the Hub log), and no harness-level error named the document's content, so FR-010a is intact. The report-naming half does not: the report silently dropped its `Review Candidates` subsection instead of naming it (or anything else) as skipped with a reason, which is what FR-010 and LINT's own degradation section (T011) both require. In practice the run needed little from the stripped section anyway — `LintAgent/system-prompt.md` already restates enough of the inbound-link counting procedure and the review-date-writing rule to keep working from its own text — but the agent gave no sign it had noticed the canonical definition was gone before relying on that restated knowledge either way. Filed as **#254** rather than fixed here, since resolving it is itself an instruction-file change subject to the same correction-loop process as any other agent-behaviour adjustment, not a direct edit from this validation task.
  - All four scratch artifacts (the manual smoke-test source, the trimmed instance foundation document, and both wikis) live under the session's own temp directory, not committed to the repository.
...
- [X] T031 Run `/speckit-converge` against the whole feature and confirm the Definition of Done holds end to end (SC-006) — **Done 2026-09-10** (layer 9, `030-prompt-conformance-09-speckit-converge`, on top of the T023 re-capture layer). Result: **converged, zero findings appended**. Checked FR-001–FR-012, SC-001–SC-006, all twelve US1–US3 acceptance scenarios (US1: 4, US2: 3, US3: 5 including 1a), the Constitution Check table, the Agentic Boundary table, and the Test Strategy table directly against the current code — not only against tasks.md's self-reported checkmarks: `FOUND`'s Lifecycle field group and write-ownership rules, re-baselined confidence thresholds; `INGEST`'s `sources/<slug>.md` requirement (no-canonical-URI and already-recorded-source cases covered) and the `10–15` wording byte-identical with `FOUND`; `LINT`'s definite `last_reviewed` write, the D5 qualifying-act rule with both FR-002d/e exclusions, and the FR-010 degradation section; `QUERY`'s lifecycle fields at creation and the reconciled FR-006a confidence rule with no aggregation prescription. `FoundationPromptSilentDefinitionTests.cs` (T002) exists; `test-fast.sh` is green; all 17 `FoundationPrompt*` integration tests pass; `dotnet test Grimoire.AgentEvals` is green at 54/0 post-recapture; `EvalRunner status` exits 0. **T025 is not run by this task** — it remains its own open item; converge does not duplicate an already-tracked, accurately-described gap into a new one, per its append-only, findings-only contract.
...
> T031, run afterward on its own layer (`030-prompt-conformance-09-speckit-converge`, PR #250).
