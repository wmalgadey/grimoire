...
- [ ] T023 Re-capture every flagged scenario against a live provider (`EvalRunner capture --scenario <id>`, or the `eval.yml` workflow **with an explicit `scenarios` input**), then run `dotnet test backend/tests/Grimoire.AgentEvals` and re-run `status`; done when the replay suite is green across all four ADR-033 classes — `IngestReplayEvalTests`, `LintReplayEvalTests`, `QueryReplayEvalTests`, `RemediationReVerificationEvalTests` — and `status` exits 0 with no scenario left stale (SC-001, FR-012) — **BLOCKED 2026-09-10, needs a live provider.** The implementing session has no eval credential: `EvalProviderResolver` resolves to `Skipped` because neither provider variable is set, so `capture` cannot run. T022 has established exactly which scenarios are stale (all 8) and why. This is the feature's one remaining real cost and it must be paid before the DoD holds. **Re-capture all nine recordings, not the eight `status` lists** — the ninth, `lint-at-scale-survey-tight-budget`, is asserted by its own test (`LintReplayEvalTests.SC003_AtScaleSurveyTightBudget_HasTrustedRecordedEvidence`) and is invisible to `status`. Run `capture --scenario <id>` for each, then `dotnet test backend/tests/Grimoire.AgentEvals` — **that suite, not `status`, is the completion condition**; `status` exiting 0 is necessary but not sufficient. **The `eval.yml` workflow is only an equivalent if its `scenarios` input names every id explicitly, the tight-budget one included.** Omitting that input does not capture everything, despite the input's own description saying it does: with no `--scenario` filter `EvalRunner` resolves lint scenarios through `LintScenarioDefinitions.DefaultSet`, which is `[AtScaleSurvey]` alone and deliberately excludes `lint-at-scale-survey-tight-budget` (that scenario is not threshold-gated, per spec 009's 2026-09-05 clarification). A default-input workflow run therefore burns a full eval and leaves the ninth recording stale, and CI stays red on `SC003_AtScaleSurveyTightBudget_HasTrustedRecordedEvidence`. The misleading input description is filed as #248; it is not this feature's to fix.
