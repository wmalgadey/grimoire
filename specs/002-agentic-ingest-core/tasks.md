...
- [ ] T048 CRITICAL Restore the real trace contract per Constitution IV and `plan.md ## Observability > Distributed Trace Spans` (partial): keep `ingest_agent.tool_call` spans under the active `ingest_agent.model_turn` span, emit `ingest_agent.finalize_artifact` on successful runs as well as failures, and ensure rollback/finalize spans carry the required run-correlation attributes before re-running `backend/tests/Grimoire.IntegrationTests/ObservabilityTraceTests.cs`
- [ ] T049 CRITICAL Fix `ingest.agent.completed` per Constitution IV and `plan.md ## Observability > Structured Log Events` (partial): emit the real `pages_created`, `pages_updated`, and `pages_superseded` counts from the journal-backed outcome instead of collapsing all touched pages into `pages_created`, then extend the deterministic log-contract coverage for a mixed create/update run in `backend/tests/Grimoire.IntegrationTests/ObservabilityLogTests.cs`
