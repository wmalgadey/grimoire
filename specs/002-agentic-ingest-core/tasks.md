...
- [ ] T034 [P] Agent-behavior evaluation SC-006 in `backend/tests/Grimoire.AgentEvals/UpdateOverDuplicateEvals.cs`: sources overlapping existing pages ⇒ ≥ 90% of sampled runs update/supersede instead of duplicating (MANDATORY — Constitution Principles II & V)
- [ ] T035 [P] Agent-behavior evaluation SC-007 in `backend/tests/Grimoire.AgentEvals/ConventionAdherenceEvals.cs`: ≥ 95% of produced/updated pages follow the active instruction conventions (deterministic frontmatter/tag checks + transcript review) (MANDATORY)
- [ ] T036 [P] Agent-behavior evaluation SC-008 in `backend/tests/Grimoire.AgentEvals/CatalogDiscoverabilityEvals.cs`: ≥ 95% of sampled successful runs leave every touched page discoverable from `index.md` (MANDATORY)
- [ ] T037 [P] Agent-behavior evaluation SC-009 in `backend/tests/Grimoire.AgentEvals/InstructionChangeAdoptionEvals.cs`: after a deliberate instruction edit (new required metadata field), ≥ 90% of subsequent sampled runs follow it with no system change (MANDATORY)
- [ ] T038 [P] Agent-behavior evaluation SC-010 in `backend/tests/Grimoire.AgentEvals/AdversarialSourceEvals.cs`: adversarial sources ⇒ 100% of out-of-scope actions denied (harness assertion) and ≥ 90% of samples still complete the legitimate update (MANDATORY)
