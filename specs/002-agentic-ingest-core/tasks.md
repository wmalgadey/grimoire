...
**Purpose**: Close the remaining metric-contract gap on the real success path and the
task-artifact body-format gap so the observability and artifact contracts fully match
`plan.md ## Observability` and `contracts/task-artifact-format.md`.

- [X] T050 CRITICAL Emit `wiki.ingest.pages_touched_total` with the plan-mandated `action=created|updated|superseded` label split derived from the journal-backed outcome on the real success path in `backend/src/Grimoire.IngestAgent/Program.cs` — the current emission uses `action="touched"`, which is outside the plan's label set, and `IngestAgentMetrics.RecordPageTouched` has no production call site — then extend `backend/tests/Grimoire.IntegrationTests/ObservabilityMetricsTests.cs` with deterministic assertions covering the action split (including the `superseded` label, currently unasserted) per plan: Observability > Business Metrics and Constitution IV (partial)
- [X] T051 Append a `## Denied actions` body section mirroring the `denied_actions` frontmatter to completed task artifacts that recorded denials, implemented in `backend/src/Grimoire.IngestAgent/TaskArtifact/TaskArtifactStore.cs` (and its call path in `backend/src/Grimoire.IngestAgent/Program.cs`), and extend `backend/tests/Grimoire.IntegrationTests/AgentTaskArtifactTests.cs` to assert the section's presence and content for a denial-bearing completed run per contracts/task-artifact-format.md body rules and US3/AC3 (partial)
- [X] T052 Review the counters `wiki.ingest.model_tool_requests_total` and `wiki.ingest.no_tool_turns_total` in `backend/src/Grimoire.IngestAgent/IngestAgentMetrics.cs`, which are not enumerated in `plan.md ## Observability > Business Metrics`: either remove them (and their call sites in `backend/src/Grimoire.IngestAgent/AgentCore/AgentLoop.cs`) or justify them and record the addition through the plan's amendment path per plan: Observability > Business Metrics (unrequested)

  Completion notes (2026-07-05): T050 — `IngestAgentMetrics` now exposes a journal-backed `RecordPagesTouched(WriteJournal)` (plus a `(action, count)` overload) emitting the `created|updated|superseded` split; `Program.cs` calls it on the success path and `RecordIngest` was reduced to `operations_total` + `duration_seconds` (the out-of-contract `action="touched"` emission and the dead `RecordPageTouched` helper are gone). `ObservabilityMetricsTests` gained a hermetic mixed create/update run test asserting the split with only plan-set labels, and a `superseded`-label test. T051 — `TaskArtifactStore.BuildMarkdown` appends a `## Denied actions` body section (action, requested/canonical target, reason, turn — mirroring frontmatter) on completed artifacts with denials; new `AgentTaskArtifactTests` asserts presence/content with denials, absence without, and narrative preservation. T052 — decision: keep and document. Both loop-diagnostic counters are recorded in `plan.md ## Observability > Business Metrics` via a dated amendment note; their deterministic tests already run in the standard CI path. Verified with the deterministic CI command set, no `ANTHROPIC_AUTH_TOKEN` set: Release build 0 errors, ArchTests 8/8, Domain.UnitTests 13/13, IntegrationTests 35/35.

## Phase 12: Convergence

**Purpose**: Close the verified gap between the hermetic-coverage obligations recorded as
complete in Phases 3–5 and the actual test inventory, plus two production-code gaps found
in the guarded-read boundary and the failed-run artifact. Constitution Principle II
(hermetic harness contracts) is the governing constraint for all test tasks; every test
below runs with `FakeModelClient` or direct component composition and no API key.

- [X] T053 Resolve symbolic links during target canonicalization in `backend/src/Grimoire.IngestAgent/Guardrails/GuardedToolExecutor.cs` so the safety policy evaluates the physical target, not the lexical path — today `Canonicalize` is lexical-only and `read_file` through an in-scope symlink pointing outside the read scope is allowed and follows the link — and add `backend/tests/Grimoire.IntegrationTests/PathTraversalTests.cs` covering, for both `read_file` and `write_file`: `../` escapes, absolute paths outside the repository, and a symlink inside the allowed scope targeting outside it, each denied with reason `traversal`/`out_of_scope` before any filesystem access per FR-007/FR-009 (contradicts)
- [X] T054 Carry the run's denial records into failed-run artifacts: `FinalizeFailedAsync` in `backend/src/Grimoire.IngestAgent/Program.cs` writes `DeniedActions: []`, losing denials on cap-breach and generic-failure paths; pass the executor's denial list where an executor exists and extend `backend/tests/Grimoire.IntegrationTests/AgentTaskArtifactTests.cs` to assert a failed artifact lists denials that occurred before the failure per FR-011 (partial)
- [X] T055 Create `backend/tests/Grimoire.IntegrationTests/InstructionLoadFailureTests.cs`: `InstructionSetLoader` returns a load failure with a human-readable reason for missing, unreadable, and whitespace-only `CLAUDE.md`; and a composed run whose instructions fail to load takes zero model turns (fake asserts never called) and performs zero wiki writes per FR-003/SC-003 (missing)
- [X] T056 Create `backend/tests/Grimoire.IntegrationTests/GovernanceIdentityTests.cs`: instruction-set loading computes `{path, sha256}` from the exact bytes loaded, the task artifact frontmatter carries those identities plus the policy `{path, version, sha256}`, and editing a `SKILL.md` between two loads changes the recorded hash per FR-012/SC-005 (missing)
- [X] T057 Create `backend/tests/Grimoire.IntegrationTests/InstructionContextTests.cs`: the system prompt captured by `FakeModelClient` contains the content of `CLAUDE.md` and every `skills/*/SKILL.md` verbatim, and the sha256 recorded for each file equals the hash of that same in-context content per FR-002 anti-loophole (missing)
- [X] T058 Create `backend/tests/Grimoire.IntegrationTests/PolicyMisconfigurationTests.cs`: `PolicyLoader` fails closed (missing file, unparseable JSON, unknown properties, `defaultDecision != "deny"` each yield a load-failure result, never a default policy), and a composed run under an all-deny write policy records every intended write as a denial visible in the run record — not a silent empty success — per FR-006 and the misconfigured-policy edge case (missing)
- [X] T059 Extend `backend/tests/Grimoire.IntegrationTests/AgentRunLifecycleTests.cs` and `AgentTaskArtifactTests.cs`: (a) mid-run failure after two writes (one updating an existing page, one creating a new file) rolls back byte-identically including deletion of the created file; (b) a turn cap lower than the scripted turns raises `AgentLoopCapException` and rollback restores the wiki; (c) the completed artifact's narrative equals the fake's final message verbatim and every path in the `pages_*` lists exists on disk per SC-004 and the T021/T022 residual obligations (missing)

  Completion notes (2026-07-05): T053 — `GuardedToolExecutor.Canonicalize` now resolves symbolic links for in-repository path segments before policy evaluation while preserving lexical targets outside the repository root, and new hermetic `PathTraversalTests` covers `../` escapes, absolute-outside targets, and in-scope symlink escapes for both `read_file` and `write_file`. T054 — failed-run artifact finalization now carries `DeniedActions` from the executor (cap-breach and generic-failure paths), with new `AgentTaskArtifactTests` coverage for failed artifacts preserving denial records. T055–T058 — added missing hermetic suites `InstructionLoadFailureTests`, `GovernanceIdentityTests`, `InstructionContextTests`, and `PolicyMisconfigurationTests` to verify instruction failure behavior, governance identity recording/hash change detection, prompt-context governance, and fail-closed policy loading plus all-deny write denials. T059 — `AgentRunLifecycleTests` now includes mid-run rollback and turn-cap rollback coverage, and `AgentTaskArtifactTests` now asserts completed-artifact narrative fidelity and path existence consistency. Verified with deterministic local gates: `dotnet build backend/Grimoire.slnx --configuration Release`, `dotnet test backend/tests/Grimoire.ArchTests --configuration Release` (8/8), `dotnet test backend/tests/Grimoire.Domain.UnitTests --configuration Release` (13/13), and `dotnet test backend/tests/Grimoire.IntegrationTests --configuration Release` (47/47), with no `ANTHROPIC_AUTH_TOKEN` required.

## Phase 13: Convergence

**Purpose**: Track SC evaluation failures that currently cannot be revalidated locally due
