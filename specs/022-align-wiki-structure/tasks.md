...
- [ ] T001 Create `backend/src/Grimoire.Hub/HarnessSurfaces/` directory for the ADR-023 options record and reserved-name declaration
- [ ] T002 [P] Add the four `Grimoire:HarnessSurfaceReads` keys (`Tasks`, `Conversations`, `Findings`, `RemediationTasks`), all `false`, to `backend/src/Grimoire.Hub/appsettings.json` with a comment citing ADR-023
- [ ] T003 [P] Create `docs/conventions/wiki-content-root.md` from `specs/022-align-wiki-structure/contracts/wiki-content-root.md` sections C1–C7 — the single authoritative layout document (SC-005)
...
- [ ] T004 Extract `FindRepositoryRoot()` into `backend/tests/Grimoire.ArchTests/ArchScan.cs` (walk up from `AppContext.BaseDirectory` until `backend/src`, `backend/tests` **and** `docs` all exist; throw naming the missing anchor otherwise) and repoint `AgentArtifactNamingRuleTests.FindConventionDocument` at it
- [ ] T005 Add `ScanTarget` record `(string RelativePath, string Text)` and a shared file enumerator to `backend/tests/Grimoire.ArchTests/ArchScan.cs`, excluding `bin/`, `obj/`, `node_modules`, `.svelte-kit`, `.git`, `.grimoire`, `frontend/`, and `backend/tests/Grimoire.AgentEvals/Fixtures/recordings/` (research.md R8)
- [ ] T006 Create `backend/tests/Grimoire.ArchTests/RetiredPagesWrapperPathRuleTests.cs` replacing `PagesWrapperRetirementBoundaryRuleTests.cs`: scan `backend/src/**/*.cs`, `backend/src/*/Instructions/**/*.md`, `backend/src/**/*.json`, `docs/**/*.md`, and repo-root `README.md`/`CONTRIBUTING.md`/`CLAUDE.md` for `pages/`, `/pages`, exact `pages`, `--pages-dir`; report violations as repo-relative `path:line`
- [ ] T007 Delete the dead `PagesDirSymbolPattern` regex and the vacuous `AllowedRelativeFilePaths` exemption array while porting — `PagesDir` no longer appears anywhere in `backend/src` (research.md R8)
- [ ] T008 Implement the historical-marker exemption in `RetiredPagesWrapperPathRuleTests`: a line under `docs/adr/` or `specs/` is exempt when it or its immediate context carries a retirement marker (`retired`, `superseded by`, `As of 014-wiki-storage-restructure`), so ADR-015 and ADR-016 pass **without being edited** (FR-010, SC-004)
- [ ] T009 Add permanent probe `[Fact] Rule_DetectsAViolation_WhenOneIsIntroduced()` to `RetiredPagesWrapperPathRuleTests` feeding a synthetic `ScanTarget` containing `` Use `list_files("pages/")` to enumerate every article `` and asserting ≥1 violation naming the synthetic file
- [ ] T010 Add permanent probe `[Fact] Rule_DoesNotFlag_AnAcceptedRecordDocumentingTheRetirement()` feeding a synthetic ADR fragment carrying a retirement marker and asserting zero violations (pins FR-010)
- [ ] T011 Create `backend/tests/Grimoire.ArchTests/WikiContentTerminologyRuleTests.cs`: scan the same surface **including C# comments** (raw text, not masked code) for `\b[Pp]ages?\b`, `Pages?[A-Z]` (PascalCase identifiers), and `\bpages?_`/`_pages?\b` (snake_case), with the not-renamed allow-list from `contracts/terminology-rename-map.md`
- [ ] T012 Add permanent probe `[Fact]` to `WikiContentTerminologyRuleTests` feeding a synthetic source containing `Meter.CreateCounter<long>("wiki.ingest.pages_touched_total")` and asserting it is flagged — this is the exact tolerance the old `IsPagesPathSegment` predicate was written to grant (SC-003)
- [ ] T013 Add the exemption-fixture mirror `[Fact]` to both rule classes, failing on drift in either direction between the in-test exemption list and `docs/conventions/wiki-content-root.md`, modelled on `AgentArtifactNamingRuleTests.ExemptionFixture_MustMirror_TheConventionDocument` (SC-005)
- [ ] T014 Create `backend/tests/Grimoire.ArchTests/HarnessSurfaceScopeRuleTests.cs` implementing ADR-023 rule **H1**: no type in `Grimoire.Domain` references a configuration/options type (`IOptions<>`, `IConfiguration`, `*Options`), via Mono.Cecil IL scan following `DomainDependencyRuleTests`
- [ ] T015 Implement ADR-023 rule **H2** in `HarnessSurfaceScopeRuleTests`: the four reserved surface names appear as string literals in exactly one production file (`ReservedHarnessSurfaces`), following ADR-022's R2 tripwire idiom
- [ ] T016 **Red/Green probe ceremony (manual, record the output in this file)** for H1: add an `IOptions<>` reference to `Grimoire.Domain/Guardrails/SafetyPolicy.cs`, run `dotnet test backend/tests/Grimoire.ArchTests`, confirm failure names that file, revert, confirm pass
- [ ] T017 **Red/Green probe ceremony (manual, record the output in this file)** for the path rule: change `list_files(".")` to `list_files("pages/")` in `backend/src/Grimoire.QueryAgent/Instructions/system-prompt.md`, run the ArchTests project, confirm failure names that file, revert, confirm pass
- [ ] T018 Verify `.github/workflows/ci.yml:37` runs `Grimoire.ArchTests` so all rules above inherit CI enforcement with no new pipeline step, and record that verification here (FR-011, SC-003)
