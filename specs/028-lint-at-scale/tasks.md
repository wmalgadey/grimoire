...
- [x] T003 [US1] **Done.** Recording captured (`lint-at-scale-survey-tight-budget`, 10 samples, `claude-haiku-4-5`, 2026-09-05) and the replay assertion implemented in `backend/tests/Grimoire.AgentEvals/LintReplayEvalTests.cs` as `SC003_AtScaleSurveyTightBudget_HasTrustedRecordedEvidence` (FR-008, SC-003). The assertion first shipped as a 90 % success-rate gate and failed at 70.0 % — the agent finds every seeded defect in 10 of 10 samples but stays under the halved budget in only 7 of 10. That gate was not what SC-003 asked for: the spec requires a *relation* across more than one point, and the scenario's own definition states no cross-scenario comparison is enforced by the scorer. `/speckit-clarify` on the spec layer (2026-09-05, commit `50123f6` on `claude/sdd-lint-at-scale-okufii`) split SC-003 into a deterministically gated clause — each point has recorded evidence that exists and stays trusted — and an observational one, the relation across points, matching SC-006's existing treatment. The test now asserts the gated clause and stops there; Red/Green probed (removing the recording fails it with `Missing`). `AtScaleSurveyTightBudget` stays out of `LintScenarioDefinitions.DefaultSet` for a new reason: `replay` gates on `IsTrustedPass`, which folds in `ThresholdMet`, so including it would reimpose the gate the clarification removed. `Grimoire.AgentEvals` is 53/53 green.
