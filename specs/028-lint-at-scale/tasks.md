...
- [ ] T003 [US1] **Capture done; the criterion it verifies is NOT met.** The recording (`lint-at-scale-survey-tight-budget`, 10 samples, `claude-haiku-4-5`, captured 2026-09-05) now exists and is Trusted, and the replay assertion `SC003_AtScaleSurveyTightBudget_ReplaysAtThreshold` is implemented in `backend/tests/Grimoire.AgentEvals/LintReplayEvalTests.cs`. It **fails**: success rate **70.0 % against the scenario's 90 % threshold** (FR-007, FR-008, SC-003, SC-006). This is a real result on recorded evidence, not an infrastructure or trust failure — all 10 samples replay as Trusted. **Which half breaks is unambiguous**: `survey_still_finds_defects` is `true` in **all 10** samples, while `stayed_within_context_budget` is `false` in samples 03, 08 and 10. Narrowing does not degrade the survey at half the budget — it simply does not narrow *enough*, reliably: the agent still finds every seeded defect but overshoots a 10k content-read budget in 3 runs out of 10. The property holds at 20k (`lint-at-scale-survey` passes) and does not generalize to 10k at this threshold with this model. Deciding what follows — accept a lower threshold for the tighter budget, treat 10k as out of the supported envelope, or change the narrowing behavior via instruction files (Principle V: a wiki-behavior change is an instruction-file change, not backend code) — is a **spec-level requirements decision** and belongs in `/speckit-clarify` on `spec.md`, not a hand-edit here (CLAUDE.md: a requirements change discovered downstream goes through clarify on the spec layer). Until then `Grimoire.AgentEvals` is red on this one assertion, and the DoD's "CI/CD pipeline passes" bullet stays open — now for a substantive reason rather than a missing credential.
...
- [x] T041 Agent-behavior evaluation completeness audit (MANDATORY — Constitution Principles II, III & V): confirmed this feature has **no high-stakes** agent-judgment success criteria of its own — SC-004, SC-005, and SC-009 are all classified lower-stakes (Constitution v1.12.0). SC-002/coverage-computation is pure harness mechanics (T004/T005), not agent judgment at all. SC-009's deviation signal is likewise pure harness mechanics (T020/T021). SC-004/SC-005 remain correction-loop-only, per T034 — T035/T036 (their optional recorded-replay checks) are **not** a DoD gap: spec.md says explicitly their absence does not fail the Definition of Done, and the constitution's correction-loop carve-out (Principle II) applies to them by name. That much is fully compliant, not merely deferred.
  ...
  - **Newly discovered while stacking Layer 3/US3 onto CI**: FR-015's `write_file` `mode: "prepend"` change (T023-T026) rewrote the `log.md`-writing instructions in all three agents' `system-prompt.md` files (Ingest, Query, Lint). `Grimoire.AgentEvals`' staleness fingerprint includes each recording's `system_prompt` hash, so this correctly invalidated the trust of **every pre-existing recorded-replay scenario for all three agents**, not only the one Lint scenario anticipated above: `remediation-reverify-still-applicable`, `remediation-reverify-no-longer-applicable`, `remediation-body-edit-applied`, `lint-at-scale-survey` (Lint); `query-synthesis-decline-edit-request`, `query-read-only-decline` (Query); `instruction-change-adoption`, `adversarial-source` (Ingest) — 8 scenarios, 9 failing assertions in CI. These scenarios verify success criteria belonging to **other, already-shipped features** (Remediation Re-Verification, Query Synthesis, Ingest instruction handling), not this feature's own SC-004/SC-005/SC-009 — this audit has no visibility into whether any of them are classified high-stakes for their owning feature, and does not assert either way. This is the correct, intended behavior of the staleness mechanism itself (the old recordings depict agent behavior — reading `log.md` and reproducing its content — that the new prompt text no longer asks for), not a bug in the mechanism or in this feature's code — but it is a real, current gap in CI's automated verification of other features' criteria, caused by this feature's change, not something this feature's own lower-stakes classification can excuse. Refreshing them requires the same credentialed capture step, run by a human. **One invocation covers all of them** — `--scenario` is repeatable and a single flat list feeds all four scenario families (Ingest/Query/Lint/Remediation), so the eight above plus T003's still-uncaptured `lint-at-scale-survey-tight-budget` refresh in one command. `--parallel 8` runs eight *samples of a scenario* concurrently (default 4, max 16) — that is where the wall-clock goes, at `GRIMOIRE_EVAL_SAMPLES` samples per scenario, default 10; the scenarios themselves still run sequentially within the process (`foreach ... await`). Do **not** work around that by launching several `capture` processes at once: the generated `lint-at-scale` fixture is shared on disk and guarded only by an in-process lock (T045), so concurrent processes reintroduce exactly the race T045 fixed, on top of contending for the same recording store. **Credential correction**: `EvalProviderResolver.Resolve` accepts *either* `ANTHROPIC_AUTH_TOKEN` *or* the complete `GRIMOIRE_EVAL_PROVIDER_BASE_URL`/`_MODEL`/`_API_KEY` triple — exactly one of the two, never both — and `capture` applies the repo-root `.env` first. The gate is therefore not specifically blocked on `GRIMOIRE_EVAL_PROVIDER_API_KEY` as stated throughout this audit; an operator whose `.env` already carries `ANTHROPIC_AUTH_TOKEN` can run this today. The argument list below was verified to parse and resolve all nine ids via the no-provider-call `status` subcommand; the capture itself was deliberately not run, since it makes live, billable provider calls:
    ```
    dotnet run --project backend/tests/Grimoire.EvalRunner -- capture --parallel 8 \
      --scenario remediation-reverify-still-applicable \
      --scenario remediation-reverify-no-longer-applicable \
      --scenario remediation-body-edit-applied \
      --scenario lint-at-scale-survey \
      --scenario query-synthesis-decline-edit-request \
      --scenario query-read-only-decline \
      --scenario instruction-change-adoption \
      --scenario adversarial-source \
      --scenario lint-at-scale-survey-tight-budget
    ```
...
- [x] T045 Fix a shared-fixture race in `Grimoire.AgentEvals` that made CI red for a reason T041's audit did not cover. `LintAtScaleFixtureTests.Ensure_IsDeterministic_AcrossRepeatedGeneration` forced a full regeneration by recursively deleting the shared `Fixtures/lint-at-scale/` tree, while every other test class reaches that same generated tree through `EvalPaths.FixtureWikiRoot` → `LintAtScaleFixture.Ensure`. xUnit runs collections in parallel, so the delete raced concurrent regeneration: `IOException: Directory not empty`, intermittently failing `Ensure_IsDeterministic_AcrossRepeatedGeneration` and `Ensure_ProducesAWikiLargerThanTheScenarioContextBudget` (both pass in isolation). Two changes: `LintAtScaleFixture.Ensure` now serializes on a process-wide lock — which is what actually makes the "no caller can observe or hash a half-built tree" guarantee already documented on `EvalPaths.FixtureWikiRoot` true — and the determinism test generates into a private temp fixtures root seeded with a copy of `lint-seeded-defects`, so its regeneration is unobservable to other classes and the assertion additionally proves determinism *across* roots rather than only within one. Verified: `AgentEvals` now fails a stable 9/52 over 5 consecutive full runs (previously a fluctuating 10), and those 9 are exactly the credential-blocked staleness failures T041 enumerates — no `LintAtScaleFixtureTests` failure remains.

- [x] T046 Refresh every stale recording via the consolidated capture command in T041 (run 2026-09-05, `claude-haiku-4-5`, 10 samples each, ~10 min wall-clock). All nine scenarios are now Trusted, and `Grimoire.AgentEvals` went from 43/52 to **52/52 green** on the pre-existing suite — the eight cross-feature staleness failures T041 recorded (Remediation ×3, Query ×2, Ingest ×2, Lint ×1) plus `EvalIndependenceFromHubConfigurationTests` are all resolved. Two follow-ons: `AtScaleSurveyTightBudget` moved into `LintScenarioDefinitions.DefaultSet`, which its own exclusion comment named as the condition for rejoining ("once a recording exists it can move into this set"); and T003's replay assertion was implemented, which surfaced the unmet-threshold finding recorded there. **T041's credential premise was wrong**: `EvalProviderResolver.Resolve` accepts `ANTHROPIC_AUTH_TOKEN` as an alternative to the `GRIMOIRE_EVAL_PROVIDER_*` triple, and the repo-root `.env` already carried one — the capture was never actually blocked, it had simply never been attempted.
