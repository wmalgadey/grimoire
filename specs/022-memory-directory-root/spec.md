...
- What happens to an operator's existing environment-variable overrides once the configuration file is restructured and their key names change? An unrecognized configuration key is silently ignored, so such an override stops taking effect and the location quietly resolves to its default. The hub does not detect or reject superseded keys: the project is pre-1.0 (alpha) with no external installations carrying old key names to protect, so there is no superseded configuration to guard against, and adding detection machinery for a case that cannot occur is not justified (see Assumptions).
...
- **Provenance of FR-013 (author directive, 2026-08-11)**: this requirement was added after the clarification session, on the author's direction that the configuration file should reflect the real directory structure rather than presenting a flat list. An earlier draft paired FR-013 with a companion requirement (FR-014) mandating startup detection of superseded configuration keys, reasoning that a renamed configuration key — unlike a removed command-line switch — fails silently unless the hub is made to detect it. That companion requirement was withdrawn on 2026-08-11: the project is pre-1.0 (alpha) with no external installations carrying old key names, so there is no superseded configuration to detect, and an unrecognized legacy key is left to the configuration system's ordinary silent-ignore behavior.
...
- **SC-008's verification mechanism (revised 2026-08-12, convergence task T066)**: SC-008 was originally verified by a dedicated deterministic test (`InstructionFilesWikiScopeTests`) that string-matched forbidden substrings in the real `system-prompt.md` files. Constitution v1.11.0 Principle V — which landed before this spec was authored and therefore binds it in full — states that "a deterministic test that string-matches required or forbidden content in a real `system-prompt.md` or `default-user-prompt.md` ... MUST NOT be added," since instruction-file content correctness is exclusively the domain of Principle II's agent-behavior evaluation tests. That test has been removed. SC-008's criterion stands, but going forward it is corroborated only by: (a) [ADR-024](../../docs/adr/ADR-024-memory-directory-root.md) rule M3, which structurally bans the wikilink *form* of these references (`[[tasks/`, etc.) in production code, narrower than SC-008's full prose scope; and (b) the FR-002/FR-012 agent-behavior regression criterion's existing evaluation-scenario thresholds, which are sampled evidence rather than an exhaustive guarantee. This is an accepted, narrower guarantee than SC-008's literal "0%" wording implies — the alternative (keeping the lexical test) would be a standing constitution violation.
