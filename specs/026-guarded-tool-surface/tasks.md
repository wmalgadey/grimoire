...
- [ ] T065 Update `backend/src/Grimoire.LintAgent/Instructions/system-prompt.md` for the agent-side judgment the new capabilities need (FR-023, Principle V): when to search vs. read, when a body edit is warranted, when to delete rather than supersede, when to reconcile the index, and what to leave as a remediation task. **Instruction-file content is never asserted by a harness test** — only its load mechanism. **Blocked, confirmed again in this layer (PR #178)**: a draft rewrite was written and then reverted after discovering it changes `system-prompt.md`'s SHA256, which feeds the eval staleness fingerprint (`Fingerprints.Compute`) the same way T009/T012 already documented for `policy.json`/`LintToolRegistry` — landing it alone would stale all 7 existing checked-in Lint/Remediation recordings (`lint-defects-found`, `lint-genuine-findings`, `lint-inbound-links-refreshed`, `lint-metadata-proposals`, `lint-remediation-proposals`, `remediation-reverify-still-applicable`, `remediation-reverify-no-longer-applicable`) and fail `Deterministic Backend Gates`' "Run replay agent evals (hermetic, zero skips)" step (`ThresholdMet: false` per `ReplayPipeline`, not a skip). T065, T009, and T012 are one inseparable unit with T068 — none of the three can land without the recapture, confirmed independently for all three now, not assumed for T065
...
- [ ] T068 Capture recordings for T066/T067 into `backend/tests/Grimoire.AgentEvals/Fixtures/recordings/`, recording the one-time eval re-capture (ADR-012). **Still blocked**: no `ANTHROPIC_AUTH_TOKEN` in this environment. This is the one task every other item in this list (T004-T006, T009, T012, T065-T067) is downstream of — none of them can land independently of it, confirmed by direct attempt in this layer (see T065's note)
- [ ] T069 Record the SC-014 before/after numbers from `wiki.read.invocations_total{shape}` in `specs/026-guarded-tool-surface/baseline.md` and the implementation PR description (SC-014, research.md D9) — a measurement, not a gate. **Blocked, transitively**: the "after" number requires a live Lint run exercising ranged reads, which requires the T009/T012 switch-flip, which requires T068. No `baseline.md` was created — recording an "estimate" with no live run behind it would misrepresent a measurement task as done; better to leave it honestly undone than fabricate a number
...
- [X] T071 Agent-behavior evaluation completeness audit against `specs/026-guarded-tool-surface/spec.md` (MANDATORY — Principles II & V): confirm SC-011 and SC-013 each have a passing evaluation at their threshold, and that **no agent-judgment criterion in spec.md lacks one** — SC-012/SC-014 are withdrawn as criteria, so the set is exactly two. **Result: gap confirmed, not closed.** Neither SC-011 nor SC-013 has any evaluation, passing or otherwise — T066/T067/T068 are blocked (see above), so no scenario exists to run. **This means the feature's Definition of Done is not met as of this layer**: "Agent-behavior evaluation tests pass for every agent-judgment success criterion" is a DoD line item this feature cannot currently satisfy. Recording this plainly rather than declaring the feature done is the point of running this audit at all
...
**Layer 08 shipped audit-only, not the full Phase N.** T009 (`policy.json` v2), T012
(`LintToolRegistry` switch-flip), T065 (`system-prompt.md` rewrite), T066/T067 (new eval
scenarios), and T068 (capturing recordings for them) are one inseparable unit: landing any
one of the first three alone stales every existing checked-in Lint/Remediation eval recording
(both change the staleness fingerprint `Fingerprints.Compute` hashes), and T066/T067 cannot
be captured without a live model. None of the five can land in an environment with no
`ANTHROPIC_AUTH_TOKEN` — confirmed by direct attempt in this layer, not just assumed. What
layer 08 could complete without live-model credentials: T063/T064 (CI-enforcement
confirmation), T070 (observability audit, no gap found), T071 (agent-behavior eval audit —
gap confirmed and recorded, not closed), and T072 (ADR audit, no gap found). **The feature's
Definition of Done is not met as of layer 08** — T071's own finding is that it cannot be,
until the credential-blocked bundle above lands in an environment that has them.
