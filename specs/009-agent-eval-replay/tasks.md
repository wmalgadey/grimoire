...
**Input**: Design documents from `/specs/009-agent-eval-replay/`
...
- [X] T002 Finish `backend/src/Grimoire.EvalRunner/Grimoire.EvalRunner.csproj` project setup: net10.0 console, reference `Grimoire.IngestAgent` (for the `IModelClient` port + replay schema types only — C7 keeps concrete adapters out), OpenTelemetry packages per `Directory.Packages.props`, nullable enabled per `backend/Directory.Build.props`; verify `dotnet build backend/Grimoire.slnx` passes with the arch rules from T001
- [X] T003 [P] Document the new environment contract in `.env-example`: commented `GRIMOIRE_MODEL_REPLAY_PATH` / `GRIMOIRE_MODEL_CAPTURE_PATH` entries with a warning that both-set is a fail-fast configuration error and that production leaves both unset (contracts/recording-format.md)
...
- [X] T004 [P] Implement the recording schema types (`RecordedSample`, `RecordedTurn`, `JudgeVerdict`, JSON (de)serialization with `schema_version`) in `backend/src/Grimoire.IngestAgent/AgentCore/Adapters/Replay/RecordingSchema.cs` per `contracts/recording-format.md` (responses verbatim; conversation requests as SHA-256 hashes)
- [X] T005 Implement `TurnCaptureModelClient : IModelClient` (decorator: forwards to inner client, appends each turn — request hashes + verbatim response — to the file named by its capture path) in `backend/src/Grimoire.IngestAgent/AgentCore/Adapters/Replay/TurnCaptureModelClient.cs` (depends on T004)
...
- [X] T036 [P] Observability implementation completeness pass: confirm every signal from `plan.md ## Observability` is emitted within span context with `task_id` correlation across `Capture/CapturePipeline.cs`, `Replay/ReplayPipeline.cs`, `Recording/StalenessCheck.cs`, `EvalRunnerTelemetry.cs` (MANDATORY — Constitution Principle IV)
...
## Implementation Completion Notes (2026-07-23)

40/43 tasks complete. The three open tasks are blocked on a live provider credential,
which this implementation environment intentionally does not hold:

- **T031 (bootstrap capture)**: run `dotnet run --project backend/src/Grimoire.EvalRunner -- capture`
  locally with the NIM proxy configured (007 quickstart), or dispatch `eval.yml` and
  commit the recordings artifact to `data/evals/recordings/`. Until then the six
  per-scenario `ReplayEvalTests` fail in CI with the actionable "recording missing"
  message — that red state is the designed FR-009/FR-016 behavior, not a defect.
- **T040 (agent-behavior verification)**: performed on T031's capture summary + the
  then-green replay tests; thresholds are asserted by `ReplayEvalTests` automatically.
- **T042 (quickstart validation)**: §3/§4 mechanics are covered hermetically by
  `StalenessTests`/`ReplayContractTests` and were exercised locally; §1/§2/§5 need the
  genuine recordings/provider from T031.

Deviation note (T024): the replay-contract fixtures are generated in-test by
`SyntheticRecordings` (temp stores) instead of committed files — same guarantee
