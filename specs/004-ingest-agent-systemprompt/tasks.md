...
- [ ] T001 Write and verify the ADR-008 structural boundary test in
...
- [ ] T002 Verify feature-003 prerequisites exist in the working tree
...
- [ ] T003 Confirm existing test gates are green before changes: run
...
- [ ] T004 Rework agent CLI options in
...
- [ ] T005 Update the Hub dispatcher invocation in
      ...
      injection unchanged.
- [ ] T006 [P] Update `Grimoire.AgentEvals` invocation plumbing in
...
- [ ] T009 [US1] Author `agents/ingest/system-prompt.md` merging the full behavioral
...
- [ ] T010 [US1] Replace `InstructionSetLoader` with `SystemPromptLoader` in
...
- [ ] T011 [US1] Record the single instruction entry (path + SHA-256, existing list
      shape) in `backend/src/Grimoire.IngestAgent/TaskArtifact/TaskArtifactDocument.cs`
      / `TaskArtifactStore.cs` (FR-004).
- [ ] T012 [US1] Adapt log events `ingest.instructions.loaded` /
...
- [ ] T013 [US1] Deterministic log/trace contract tests for T012 in
...
- [ ] T014 [US1] Update eval fixtures to the single document:
...
- [ ] T015 [P] [US2] New integration tests in
      ...
      still denied).
- [ ] T016 [P] [US2] New Hub API tests in
...
- [ ] T017 [US2] Create `agents/ingest/default-user-prompt.md` with the steering text
...
- [ ] T018 [US2] Rework `backend/src/Grimoire.IngestAgent/AgentCore/AgentLoop.cs` +
      ...
      FR-009).
- [ ] T019 [US2] Emit `ingest.agent.user_prompt_resolved` (INFO; `task_id`,
...
- [ ] T020 [US2] Hub: defaults endpoint + prompt intake in
...
- [ ] T021 [US2] Hub observability: metric `wiki.ingest.user_prompt_total`
...
- [ ] T022 [US2] Deterministic log/metric contract tests for T019/T021 in
      `backend/tests/Grimoire.IntegrationTests/ObservabilityLogTests.cs` and
      `ObservabilityMetricsTests.cs`.
- [ ] T023 [US2] Frontend: prompt editor prefilled from defaults endpoint in
...
- [ ] T025 [P] [US3] New integration tests in
...
- [ ] T026 [US3] Convert-step registry (name, appliesTo, requiredFor, defaultEnabled)
...
- [ ] T027 [US3] Skip path in
      ...
      (FR-012, FR-014).
- [ ] T028 [US3] API fields `convertSteps` on POST/detail/board responses in
      `backend/src/Grimoire.Hub/IngestSubmission/IngestSubmissionEndpoints.cs` per
      contract.
- [ ] T029 [US3] Hub observability: metric `wiki.ingest.convert_step_disabled_total`
...
- [ ] T033 [US4] Test support: fake agent executable + scripted NDJSON event-stream
...
- [ ] T034 [P] [US4] Supervision tests in
...
- [ ] T035 [P] [US4] Queue tests in
      ...
      (SC-010).
- [ ] T036 [P] [US4] Realtime propagation test in
...
- [ ] T037 [US4] Agent event emission: new
...
- [ ] T038 [US4] Loop activity events: emit `activity` (modelTurns, toolCalls,
...
- [ ] T041 [US4] Non-blocking queue-driven dispatch: replace
...
- [ ] T042 [US4] Queue/activity API surface in
      `backend/src/Grimoire.Hub/IngestSubmission/IngestSubmissionEndpoints.cs`:
...
- [ ] T043 [US4] Deterministic log/metric/trace contract tests for T039–T041 rows in
      `backend/tests/Grimoire.IntegrationTests/ObservabilityLogTests.cs`,
      ...
      `task_id` correlation).
- [ ] T044 [US4] Frontend: live activity in the task detail view
      ...
      two new endpoints.
- [ ] T045 [P] [US4] Frontend tests: activity rendering, queue position, paused
...
- [ ] T046 Observability completeness audit (MANDATORY — Principle IV): verify every
...
- [ ] T047 CI enforcement (MANDATORY — Principle IV): confirm the logging/trace/
...
- [ ] T049 Agent-behavior evaluation SC-007 (MANDATORY — Principles II & V): new
      ...
      the steer, threshold ≥ 90%.
- [ ] T050 [P] Update stale doc references to `CLAUDE.md`/`SKILL.md`/
      ...
      instructions if they mention the old CLI flags.
- [ ] T051 Run quickstart.md Scenarios 1–10 end-to-end and record outcomes in the PR
      description.
