...
- Q: Feature 004 established a single global agent slot (only one agent runs at a
  time). Should Query runs bypass that slot entirely and run concurrently, as the
  Assumptions section proposes? → A: Yes — fully decoupled. Query runs never occupy
  or wait for the ingest single-agent slot; they run concurrently with ingest and
  with each other, governed only by their own configurable global query-concurrency
  limit.
- Q: What should the default value of the global query-concurrency limit be? → A: 3.
- Q: Since Query and Ingest now run concurrently, should a Query turn's wiki reads
  be pinned to a snapshot taken at turn start, or read live content on each tool
  call? → A: Live reads, no snapshot isolation. Each wiki read tool call returns
  current content at call time; a turn may in principle observe a page change made
  by a concurrent ingest run mid-answer. No snapshot/versioning mechanism is
  introduced for this.
- Q: Can a single browser window hold multiple concurrent Query Conversations
  (e.g. a switcher/list), or exactly one active conversation per window at a time?
  → A: One active conversation per window. Starting a new conversation (FR-010)
  replaces the current one in that window; no in-UI conversation list or switcher is
  introduced.
...
- What happens when the wiki is empty (no pages at all)? The turn runs normally and
  ...
  answer, not an error.
- What happens when two browser windows are open? Each window holds its own single
  active conversation; the one-active-turn rule applies per conversation, and a
  global concurrency limit protects the Hub (see Assumptions). A submission beyond
  that limit is rejected immediately with a clear "busy" message rather than
  silently queued.
...
- **FR-017**: Query activity MUST NOT block ingest activity and vice versa: submitting
  ...
  turns, defaulting to 3. A submission beyond that limit is rejected immediately with
  a clear message.
...
- **Queries run outside the ingest agent slot** (confirmed, see Clarifications):
  Feature 004's single-agent invariant exists to serialize wiki-writing ingest runs.
  Query runs are strictly read-only and therefore do not occupy or wait for the
  ingest slot; they run alongside ingest activity, fully decoupled from it. A
  configurable global limit on concurrent query turns (default: 3) protects the Hub;
  submissions beyond it are rejected immediately with a clear "busy" message, not
  queued.
- **One active turn per conversation**: Within one conversation the user asks, then
  ...
  scope.
- **Conversations are session-scoped in the UI, one per window** (confirmed, see
  Clarifications): Each browser window/tab holds exactly one active Query
  Conversation at a time; starting a new conversation replaces the current one in
  that window, with no in-UI conversation list or switcher. The conversation display
  lives in the browser session; persisting and restoring full conversations across
  reloads or devices is out of scope. Traceability is provided per turn via Query Run
  Artifacts, not via a conversation browser. A page reload with an in-flight turn
  treats that turn as interrupted.
...
- **"The wiki" means the wiki's current page content**: The Query agent answers from
  ...
  general web access.
- **Wiki reads are live, not snapshotted** (confirmed, see Clarifications): Each
  guarded read tool call returns current wiki content at call time; a turn's reads
  are not pinned to a fixed snapshot from turn start. Because Query and Ingest run
  concurrently, a long-running turn could in principle see a page change made by a
  concurrent ingest run mid-answer; no snapshot/versioning mechanism is introduced to
  prevent this.
